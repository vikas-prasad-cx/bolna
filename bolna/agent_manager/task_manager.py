...
class TaskManager(BaseManager):
    ...
    async def _report_component_health(self, service, provider, latency_s, connect_flag):
        """Per-turn ASR/TTS health for the shadow breaker: process latency every turn, plus the
        connection latency once (as a watch-only 'connect' phase). Never affects the call."""
        if not self.on_provider_health or not provider:
            return
        ...
        await self._report_provider_health(service, provider, model, True, latency_ms, phase="process")
            ...
            while True:
                ...
                if self.stream:
                    ...
                    # Whenever speech_final or UtteranceEnd is received from Deepgram, this condition would get triggered
                        ...
                        # Per-turn ASR health for the shadow breaker (final transcript delivered).
                        await self._report_component_health(
                            "transcriber",
                            self.transcriber_provider,
                            _meta.get("transcriber_first_result_latency"),
                            ...
                            if self.stream:
                                if meta_info.get("is_first_chunk", False):
                                    ...
                                    await self._report_component_health(
                                        ...
                                        meta_info.get("synthesizer_latency"),
